...
function emitNode(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    for (const rpc of svc.rpcs) {
      ...
      const resType = rpc.outputType?.name || "unknown";
      if (rpc.isStream) {
        client.push(`    ${rpc.name}: (req: ${reqType}) => _client.stream<${resType}>(${svc.serviceName}.methods.${rpc.name}.path, req),`);
      } else {
        client.push(`    ${rpc.name}: (req: ${reqType}) => _client.call<${resType}>(${svc.serviceName}.methods.${rpc.name}.path, req),`);
      }
    }
    client.push(`  };`);
    client.push(`}\n`);
