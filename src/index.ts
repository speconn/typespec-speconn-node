...
function emitNode(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    server.push('import { createSpeconnRouter, type RouteDefinition } from "speconn-node";');
    ...
    client.push('import { createUnaryClient, createStreamClient } from "speconn-node";');
    ...
    for (const rpc of svc.rpcs) {
      ...
      const resType = rpc.outputType?.name || "unknown";
      if (rpc.isStream) {
        client.push(`    ${rpc.name}: (req: ${reqType}) => createStreamClient<${reqType}, ${resType}>(baseUrl, ${svc.serviceName}.methods.${rpc.name}.path).call(req),`);
      } else {
        client.push(`    ${rpc.name}: (req: ${reqType}) => createUnaryClient<${reqType}, ${resType}>(baseUrl, ${svc.serviceName}.methods.${rpc.name}.path).call(req),`);
      }
    }
    client.push(`  };`);
    client.push(`}\n`);
