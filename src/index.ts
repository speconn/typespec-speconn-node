...
function emitNode(program: Program, services: ServiceInfo[], outputDir: string): Promise<void[]> {
  ...
  for (const svc of services) {
    ...
    client.push('import type { HttpClient, CallOptions } from "@speconn/node";');
    ...
    client.push(`export function create${svc.serviceName}Client(baseUrl: string, httpClient?: HttpClient) {`);
    client.push(`  const _client = createClient(baseUrl, httpClient);`);
    client.push(`  return {`);
    for (const rpc of svc.rpcs) {
      const reqType = rpc.inputType?.name || "Record<string, never>";
      const resType = rpc.outputType?.name || "unknown";
